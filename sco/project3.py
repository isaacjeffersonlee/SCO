...
def plot_implicit_MSE_analysis():
    MSEs = [[], []]
    for k in range(1, 1000):
        ...
        fI_gt = np.sin(k * ygI)
        for m in (0, 1):
            fI_pred = part2(f, method=m)
            MSE = (1 / fI_pred.size) * np.sum((fI_pred - fI_gt) ** 2)
            MSEs[m].append(MSE)

    fig, ax = plt.subplots()
    ax.plot(MSEs[0][:-1], label="method 1")  # Remove spurious boundary results
    ax.plot(MSEs[1][:-1], label="method 2")
    ax.set_ylabel("MSE")
    ax.set_xlabel("Wavenumber, $k$")
    ax.legend()


def plot_wavenumber_analysis(a, b, alpha):
    fig, ax = plt.subplots()
    kh = np.linspace(0, np.pi, 100)
    method_1_wavenumber = kh * np.cos(kh / 2)
    method_2_wavenumber = kh * (
        b * np.cos((3 / 2) * kh) + a * np.cos((1 / 2) * kh) - 2 * alpha * np.cos(kh)
    )
    ax.plot(kh, kh, label="exact")
    ax.plot(kh, method_1_wavenumber, label="method 1")
    ax.plot(kh, method_2_wavenumber, label="method 2")
    ax.set_xlabel("kh")
    ax.set_ylabel("kh'")
    ax.legend()
...
def part2_analyze():
    # plot_implicit_MSE_analysis()
    plot_wavenumber_analysis(a=1.5, b=0.1, alpha=0.3)
    # plot_walltime_vs_m()
    plt.show()
    return None
...
def part3_analyze(display_1=False, display_2=False, display_3=False, display_4=False):
    """
    ...
    # Calculate dominant frequency of first component using welches method.
    if display_3:
        fig, ax = plt.subplots()
    ...
    if display_4:
        # De-correlate neighboring coordinates using time delays
        f = 0.1875
        tau = 1 / f
        Del = int(tau / dt)
        u_filtered = u[np.arange(0, u.shape[0], Del), :]
        D = pdist(u_filtered)
        ...
        coeffs = np.polyfit(np.log(eps_vals[start_fit_idx:end_fit_idx]), np.log(C_vals[start_fit_idx:end_fit_idx]), 1)
        f = np.poly1d(coeffs)
        z = f(np.log(eps_vals[start_fit_idx:end_fit_idx]))
        fig, ax = plt.subplots()
        ax.plot(np.log(eps_vals), np.log(C_vals), label="ln $C(\\epsilon)$", marker="x")
        ax.plot(
            np.log(eps_vals[start_fit_idx:end_fit_idx]),
            z,
